...
if __name__=='__main__':
	...
	filename_hh = "results/sub-%02d/disc/predictions_varm_HH.txt"%args.subject
	filename_hr = "results/sub-%02d/disc/predictions_varm_HR.txt"%args.subject
	...
	for target_column in regions:
		...
		for  external_predictors in  behavioral_predictors:
			...
			for lag in range (1, args.lag + 1):
				...
				for i in range (4):
					model = train_global_model (convers[i:i+4], target_column, target_index, subject, predictors, external_predictors, lag, epochs=500, add_target = False)
					
					for conv in convers[i+4:i+6]:
						score = test_global_model (model, conv, target_index, target_column, subject, predictors, external_predictors, lag, epochs=10, add_target = False)
				...
				if lag == 1:
					...
					if (results[0] + results[1]) < (min_result[0] + min_result[1]):
						min_result = results[:]
						best_lag = lag
