...
class toSuppervisedData:
	...
	## constructor
	def __init__(self, X, p, test_data = False, add_target = True, nb_inter = 0):
		...
		if X.shape[1] > 1 and p > 4:
			delete = []
			for j in range (0, nb_inter * p, p):
				delete. append (j)
				delete. append (j+1)
				delete. append (j+2)
				delete. append (j+3)

			for j in range (nb_inter * p, self.data. shape [1], p):
				delete. append (j+0)
				delete. append (j+2)
				delete. append (j+3)
				delete. append (j+4)

			self.data = np. delete (self.data, delete, axis = 1)
