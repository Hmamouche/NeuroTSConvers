...
def process_multiple_subsets (set_of_behavioral_predictors, convers, target_column,subject, predictors, behavioral_predictors_,lag_max, k, add_target, model, filename, colnames, write_scores):
	...
	for behavioral_predictors in set_of_behavioral_predictors:
		best_model_params, score = k_l_fold_cross_validation (convers, target_column,
		subject, predictors, behavioral_predictors,
		lag_max = lag_max, k = k, add_target = add_target, model = model)
...
def concat (subject, target_column, convers, lag, behavioral_predictors, predictors, add_target):
	...
	for conv in convers:
		...
		if behavioral_predictors != None:
			...
			if data.shape [0] == 0:
				data = pd. concat ([target, physio], axis = 1)
			else:
				U = pd. concat ([target, physio], axis = 1)
				data =  pd. concat ( [data, U], axis = 0)

	supervised_data = toSuppervisedData (data.values, lag, add_target = add_target)

	return pd.DataFrame (supervised_data.data)
...
def predict_regions (regions, convers, subject, lag_max, k, model, model_type, filename, colnames, write_scores):
	...
	for target_column in regions:
		...
		if model_type == "multiv":
			...
			if target_column in ["region_3"]:
				predictors = []
				behavioral_predictors_ = [{"speech_left_ts": ["IPU"]}]

			if target_column in ["region_2"]:
				predictors = []
				behavioral_predictors_ = [{"speech_left_ts": ["IPU"]}]
				...
				predictors = []
				behavioral_predictors_ = {"speech_ts": ["IPU"]}
			...
			'''best_model_params, score = k_l_fold_cross_validation (convers, target_column, target_index,
														  subject, predictors, behavioral_predictors_,
														  lag_max = lag_max, k = k, add_target = add_target, model = model)'''
		...
		'''else:
			...
			best_model_params, score = k_l_fold_cross_validation (convers, target_column, target_index,
														  subject, predictors, behavioral_predictors_,
														  lag_max = 1, k = k, add_target = True, model = model)

		if write_scores:
			row = [subject, target_column,'', best_model_params, str (behavioral_predictors_)] + score
