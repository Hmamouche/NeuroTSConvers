...
if __name__=='__main__':
	...
	if args.pred_type:
		...
		filename_hh = "results/sub-%02d/disc_all/multivariate_physio_randF_HH.csv"%args.subject
		filename_hr = "results/sub-%02d/disc_all/multivariate_physio_randF_HR.csv"%args.subject
		...
		filename_hh = "results/sub-%02d/multivariate_physio_randF_HH.csv"%args.subject
		filename_hr = "results/sub-%02d/multivariate_physio_randF_HR.csv"%args.subject
	...
	convers_test = convers[-nb_pred:]
	convers = convers [0:-nb_pred]
	...
	for target_column in regions:
		...
		for  external_predictors in  behavioral_predictors:
			...
			for lag in range (1, args.lag + 1):
				...
				convers_order = rd. sample (range (nb_test), k = nb_test)
				...
				if lag == 1:
					...
					if ((results[0][2] + results[1][2]) > (best_result[0][2] + best_result[1][2])):
						best_result = results[:]
						best_lag = lag

			if len (best_set_result) == 0 or ((best_result[0][2] + best_result[1][2]) > (best_set_result[0][2] + best_set_result[1][2])):
